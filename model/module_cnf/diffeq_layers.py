...
class SpkEmoLinear(nn.Module):
    def __init__(self, dim_in, dim_out, dim_spk, dim_emo):
        super().__init__()
        self._layer = nn.Linear(dim_in, dim_out)
        
        self._hyper_bias = nn.Linear(1 + dim_spk + dim_emo, dim_out, bias=False)
        self._hyper_gate = nn.Linear(1 + dim_spk + dim_emo, dim_out)
        
    def forward(self, x, t, spk_emb, emo_emb):
        """
        #=== INPUT
        * x: (batch, dim_noise)
        * t: (batch, 1)
        * spk_emb: (batch, dim_spk)
        * emo_emb: (batch, dim_emo)
        """
        
        _cat = torch.concatenate([spk_emb, emo_emb, t], dim=-1)
        
        gate = torch.sigmoid(self._hyper_gate(_cat))
        bias = self._hyper_bias(_cat)
